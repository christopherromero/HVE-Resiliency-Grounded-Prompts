...
| Target deployment | Active-active application behavior; Kafka provisionally active-standby |
...
* Regional determinism is not established because repository defaults can select one region's endpoints and labels, and runtime images are mutable (`F-001`, `F-011`; verified).
* Required state and cross-store work are not durable across pod or regional movement (`F-003`, `F-007`; verified).
* Kafka and scheduled workloads lack fenced ownership and recoverable completion (`F-008` through `F-010`, `F-012`; `F-012` verified and Kafka scenario findings conditional).
* SQL and gateway calls lack complete deadline, isolation, and retry budgets (`F-002`, `F-005`, `F-006`; verified).
* Operational health details are exposed more broadly than required (`F-004`; verified and non-resiliency-focused).
...
These approved references provide external architectural context. They are not repository-discovered interactions or proof of deployed infrastructure compliance.

### Summary findings table

| Priority | Section | Confirmed count | Description |
|---|---:|---:|---|
| P0 | Resiliency-focused | 4 | Regional configuration, durable state, cross-store consistency, and scheduled ownership |
...
| P1 | Resiliency-focused | 3 | SQL deadlines, HTTP isolation, and retry classification |
...
**Resiliency Impact:** A second-region instance can connect to the wrong dependency or emit misleading regional telemetry.

**Recommended Fix:** Remove production-capable regional fallbacks, validate required production values, and retain local defaults only in an explicit local profile.

**File:** `source/customer-app/notification-service/src/main/resources/application.yml:9-64`
...
* Cross-refs: Source `F-001`; controls `APP-AA-002`, `APP-AA-015`, `KV-001`, `KAFKA-001`; evidence `EV-F-001-01` through `EV-F-001-03`; change `CHANGE-AA-001`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P0-AA-001`; severity Critical; status Verified finding (`non_compliant`)
* Implementation: Target symbols are `spring.kafka.bootstrap-servers`, `spring.cloud.azure.keyvault.secret.endpoint`, and `management.metrics.tags.region`; confirm the canonical region property and enumerate inventory-cited service configuration
* Validation: Start two regional configurations, verify distinct endpoint and telemetry resolution, fail startup on missing production values, and preserve the local profile
* Guardrail: Do not hardcode physical regions or modify infrastructure
...
**Recommended Fix:** Introduce a repository port and an architecture-approved durable adapter with stable identity and concurrency behavior.
...
* Cross-refs: Source `F-003`; control `APP-AA-010`; evidence `EV-F-003-01`; change `CHANGE-AA-003`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P0-AA-005`; severity Critical; status Verified finding (`non_compliant`)
* Implementation: Target symbol is `NotificationRepository.store`; architecture must approve the existing durable store, retention, and read-consistency contract
* Validation: Write through one instance, read through another, restart both, and verify stable identity prevents duplication
* Guardrail: Do not invent a store or silently fall back to process-local state
...
**File:** `source/customer-app/order-service/src/main/java/com/ecommerce/order/service/OrderService.java:100-105`
...
* Cross-refs: Source `F-007`; controls `SQL-013`, `APP-STATE-001`, `APP-DB-003`; evidence `EV-F-007-01`; change `CHANGE-AA-007`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P0-AA-006`; severity Critical; status Verified finding (`non_compliant`)
* Implementation: Target symbol is `createOrder`; approve outbox retention, terminal failure, replay, and reconciliation procedures
* Validation: Fail after SQL commit and before or during send, run concurrent dispatchers, and verify one logical event with durable recovery status
...
**Issue:** Under the inferred active-standby scenario, Kafka listeners and scheduled producers start through unconditional annotations with no regional owner check or stale-owner fence. This is conditional on GOV-001 confirming the cluster model, promotion order, offset synchronization, ownership authority, and failback contract.
...
**Resiliency Impact:** Both regions can acquire or produce authoritative work, and a stale active can continue after ownership transfer.
...
**File:** `source/customer-app/inventory-service/src/main/java/com/ecommerce/inventory/service/InventoryService.java:250-269`
...
* Cross-refs: Source `F-008`; controls `KAFKA-AS-001`, `KAFKA-AS-003`, `KAFKA-AS-007`, `KAFKA-012`, `APP-KAFKA-001`, `APP-WORKLOAD-002`; evidence `EV-F-008-01`, `EV-F-008-02`; change `CHANGE-AA-008`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P0-AA-006`; severity Critical; status Conditional finding (`non_compliant` under inferred active-standby)
* Implementation: Target symbols are `handleOrderCreated` and `expireOldReservations`; depends on `CHANGE-AA-001` and GOV-001 approval of the ownership provider
* Validation: Exercise active, standby, both-active, both-inactive, stale-owner, ownership loss, promotion, and failback
...
**File:** `source/customer-app/inventory-service/src/main/java/com/ecommerce/inventory/service/InventoryService.java:268-287`
...
public void cleanExpiredCarts() {
    log.info("Cleaning expired carts...");
...
* Cross-refs: Source `F-012`; controls `APP-SCHED-001`, `APP-SCHED-002`, `APP-WORKLOAD-001`; evidence `EV-F-012-01`, `EV-F-012-02`; change `CHANGE-AA-012`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P0-AA-006`; severity Critical; status Verified finding (`non_compliant`)
* Implementation: Target symbols are `expireOldReservations` and `cleanExpiredCarts`; depends on `CHANGE-AA-008` and approval of claim granularity, lease budget, and existing-store claim location
* Validation: Run two pods and two region identities, expire a lease, crash after claim, and verify recovery, stale-owner rejection, and duplicate prevention
...
**File:** `source/customer-app/order-service/src/main/resources/application.yml:8-17`
...
* Cross-refs: Source `F-002`; controls `SQL-002`, `APP-AA-004`; evidence `EV-F-002-01`; change `CHANGE-AA-002`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P1-RCV-001`; severity Critical; status Verified finding (`non_compliant`)
* Implementation: Target is `spring.datasource`; obtain approved caller, drain, SQL, transaction, and lock budgets and validate driver property names
* Validation: Delay acquisition, connection, statement, lock, and commit separately and verify each configured layer and overall budget
...
**File:** `source/customer-app/api-gateway/src/main/resources/application.yml:30-33`
...
* Cross-refs: Source `F-005`; controls `APP-HTTP-002`, `HTTP-003`; evidence `EV-F-005-01`; change `CHANGE-AA-005`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P1-RCV-003`; severity High; status Verified finding (`non_compliant`)
* Implementation: Target is `spring.cloud.gateway.httpclient`; approve route budgets and validate supported Spring Cloud 2023.0.1 APIs
* Validation: Saturate one route and verify unrelated routes retain bounded latency and capacity with dependency-specific metrics
...
**File:** `source/customer-app/api-gateway/src/main/java/com/ecommerce/gateway/config/GatewayRoutesConfig.java:73-77`
...
* Cross-refs: Source `F-006`; controls `APP-HTTP-003`, `HTTP-004`; evidence `EV-F-006-01`; change `CHANGE-AA-006`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P1-RCV-002`; severity High; status Verified finding (`non_compliant`)
* Implementation: Target is the `customRouteLocator` product-service-public retry; depends on `CHANGE-AA-005` and approved transient classifications and delay cap
* Validation: Exercise 429 and 503 responses, `Retry-After` variants, transport failures, terminal 4xx, and the total deadline
* Guardrail: Preserve GET-only scope and follow Gateway buffer lifecycle APIs
...
**Issue:** Under the inferred active-standby scenario, notification listeners catch side-effect failures, log them, and return normally. GOV-001 architecture confirmation and approved channel idempotency and quarantine retention are required.
...
**Recommended Fix:** Propagate retryable failures to a finite error handler and quarantine exhausted records with stable identity, or durably record retry work before success.

**File:** `source/customer-app/notification-service/src/main/java/com/ecommerce/notification/service/NotificationEventConsumer.java:45-50`
...
* Cross-refs: Source `F-009`; controls `KAFKA-004`, `APP-KAFKA-002`; evidence `EV-F-009-01`; change `CHANGE-AA-009`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P2-DATA-002`; severity Critical; status Conditional finding (`non_compliant` under inferred active-standby)
* Implementation: Target is `handleOrderCreated`; depends on `CHANGE-AA-003`, `CHANGE-AA-008`, channel idempotency, and quarantine retention
* Validation: Force each channel to fail and verify no premature commit, finite retry, quarantine, redelivery, and recovery
...
**Issue:** Under the inferred active-standby scenario, the scheduled cart producer discards the send future and logs publication before broker completion. GOV-001 architecture confirmation and approved publication-status and quarantine policies are required.
...
**File:** `source/customer-app/cart-service/src/main/java/com/ecommerce/cart/service/CartService.java:217-228`
...
* Cross-refs: Source `F-010`; controls `APP-KAFKA-005`, `KAFKA-002`; evidence `EV-F-010-01`; change `CHANGE-AA-010`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P2-DATA-002`; severity Critical; status Conditional finding (`non_compliant` under inferred active-standby)
* Implementation: Target is `detectAbandonedCarts`; depends on `CHANGE-AA-008`, `CHANGE-AA-012`, and approved publication-status and quarantine policies
* Validation: Simulate asynchronous rejection and timeout, retry with the same event ID, quarantine exhaustion, and verify no premature success log
...
**File:** `source/customer-app/api-gateway/Dockerfile:42-42`
...
* Cross-refs: Source `F-011`; control `APP-SUPPLY-002`; evidence `EV-F-011-01`; change `CHANGE-AA-011`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P0-AA-001`; severity High; status Verified finding (`non_compliant`)
* Implementation: Target is runtime `FROM`; security must approve the exact digest and targeted discovery must confirm all service Dockerfiles
* Validation: Build each service, inspect the resolved digest in both delivery paths, and run image policy and vulnerability checks
...
* Cross-refs: Source `F-004`; control `APP-ACT-003`; evidence `EV-F-004-01`; change `CHANGE-AA-004`; policy `AA-REMEDIATION-PRIORITY` version `1.0.0`, rule `P3-OPS-005`; severity Medium; status Verified finding (`non_compliant`)
* Implementation: Target is `management.endpoint.health.show-details`; confirm the operator authority and coordinate with Spring Security conventions
* Validation: Compare anonymous and authorized health responses and verify liveness and readiness status behavior
...
| `KV-004` | Key Vault is a property source rather than a per-request call |
...
| Application YAML | Eight service configurations with endpoints, clients, health, and telemetry | Establishes repository-owned defaults and budgets | `F-001`, `F-002`, `F-004`, `F-005` |
| Java source | Controllers, repositories, Kafka handlers, services, and schedules | Establishes state, completion, and ownership behavior | `F-003`, `F-006` through `F-010`, `F-012` |
...
| Azure Pipelines | Maven verify, Surefire, and JaCoCo publication | Test tooling is configured, but source-controlled tests are absent | Validation scope only |
...
| Kafka cluster and replication model | Topology, linking, writable role, offsets, promotion, and failback | `F-008` through `F-010` |
| Regional ownership authority | Fail-closed owner, epoch source, fencing, and split-brain response | `F-008`, `F-012` |
| Cosmos DB deployed topology | API/throughput confirmation, preferred regions, multi-region writes, and conflict policy | Active-active assumptions; no infrastructure finding |
| Azure Managed Redis exact assessment standard | Registry path correction or approved alias | Redis controls remain `not_assessed` |
...
|---|---|---|---|
| Spring Boot on AKS Active-Active Code Assessment Standard v4.0.0 | Evaluated; mapped noncompliance and evidence-limited controls preserved | `APP-*` | `F-001`, `F-003` through `F-012` |
| Azure Key Vault Application Behavior Standard v2.0.0 | Evaluated; 2 compliant controls and 1 mapped noncompliance | `KV-001` through `KV-016` | `F-001` |
| Azure Cosmos DB Application Behavior Standard v1.0.0 | Evaluated; all controls remain `not_assessed` | `COSMOS-001` through `COSMOS-008` | None |
...
| Confluent Kafka Multi-Region Standard v3.0.0 | Evaluated under policy-inferred active-standby; scenario findings remain conditional | `KAFKA-*`, `KAFKA-AS-*` | `F-001`, `F-008` through `F-010` |
| Spring HTTP and SDK Client Grounding v2.0.0 | Evaluated; circuit-breaker execution compliant and isolation/retry controls noncompliant | `HTTP-003`, `HTTP-004`, `HTTP-007` | `F-005`, `F-006` |
| Azure Managed Redis Application Behavior Standard | Not assessed because the exact inventory-selected path was absent | Standard-level result | None |
| Active-Active Remediation Prioritization Policy v1.0.0 | Applied without overrides; every change preserves its rule | `P0-AA-*`, `P1-RCV-*`, `P2-DATA-*`, `P3-OPS-*` | All findings |

[Back to Top](#top)

## 7. Implementation Roadmap

The authoritative Step 3A plan remains the implementation source. This section summarizes its sequence without replacing its specifications, tests, acceptance criteria, decisions, or validation commands.
...
Step 4 uses `.copilot-tracking/plans/2026-09-01/springboot-active-active-authoritative-remediation-plan.instructions.md` and supports `APPROVED_PRIORITIES`, `APPROVED_WAVES`, and `APPROVED_CHANGE_IDS`. Priorities are preferred. Step 4 freezes resolved change IDs in its implementation artifact before modifying source code. Only those resolved changes may be implemented.

Step 3B generated this report directly. Task Implementor did not generate it. After Task Planner is complete, the user switches to Task Implementor for approved Step 4 remediation. All workflow state is managed through `.copilot-tracking/` files.
