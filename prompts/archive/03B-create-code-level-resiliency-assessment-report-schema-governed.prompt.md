...
7. Verify illustrative code comes from Step 3A or is omitted.
8. Verify conditional findings identify conditions and evidence required.
9. Verify no `not_assessed` item is presented as noncompliant.
10. Verify summary counts match the Full Finding Matrix.
11. Verify the roadmap matches Step 3A priority and wave indexes.
12. Verify no infrastructure findings were created.
13. Verify no PCF findings or recommendations appear.
14. Verify internal links and anchors are valid.
15. Verify no unapproved links were invented.
16. Verify evidence and scope limitations are explicit.
17. Verify schema-conformance status is `passed`.
18. Verify the complete report was written by Step 3B.
19. Verify report generation was not delegated to Task Implementor.
20. Verify no application source, configuration, test, deployment, or infrastructure file was modified.
